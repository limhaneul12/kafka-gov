...
4. Topic/Schema 변경 시 approval, audit, policy, impact를 같은 흐름에서 다룬다.
...
- **Topic**과 **Schema**는 가장 production-ready한 모듈이다.
...
즉, 오늘의 shipped runtime은 아직 **Kafka governance platform**에 더 가깝고, 이 문서는 그것을 **real-time data governance system**으로 끌어올리기 위한 방향 문서다.
...
목표: Topic/Schema 운영 기능을 플랫폼 하부 런타임으로 고정한다.
...
4. Topic/Schema 변경이 approval/audit/governance 흐름과 연결된다.
5. 변경 영향이 raw topic name만이 아니라 Product/Contract 관점으로 설명된다.
6. 기존 Topic/Schema 운영 기능은 유지된다.
...
1. Topic/Schema application use case에서 shared adapter 직접 생성 제거
