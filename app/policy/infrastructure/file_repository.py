...
class FilePolicyRepository(IPolicyRepository):
    ...
    async def list_environments(self) -> list[DomainEnvironment]:
        ...
        for file_path in self.config_dir.glob("*.json"):
            try:
                ...
                environments.add(DomainEnvironment(env_str))
    ...
    async def list_resource_types(self, environment: DomainEnvironment) -> list[DomainResourceType]:
        ...
        for file_path in self.config_dir.glob(f"{environment.value}_*.json"):
            try:
                ...
                resource_types.append(DomainResourceType(rt_str))
            except (ValueError, AttributeError):
                continue
    ...
    def _serialize_policy_set(self, policy_set: DomainPolicySet) -> dict[str, Any]:
    ...
    def _deserialize_policy_set(self, data: dict[str, Any]) -> DomainPolicySet | None:
