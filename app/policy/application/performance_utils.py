...
if TYPE_CHECKING:
    from ..domain.models import PolicyRule, PolicyTarget, PolicyViolation
...
class PolicyEvaluationOptimizer:
    ...
    async def evaluate_rules_parallel(
        ...
        Returns:
            모든 위반 사항 목록
        """
        async def _evaluate_rule_batch(rule: PolicyRule) -> list[PolicyViolation]:
    ...
    async def evaluate_targets_parallel(
        ...
        Returns:
            위반 사항 목록
        """
        async def _evaluate_chunk(chunk: list[PolicyTarget]) -> list[PolicyViolation]:
            async with self._semaphore:
                violations = []
                for target in chunk:
                    violations.extend(rule.validate(target, context))
...
class PolicyCacheOptimizer:
    ...
    def __init__(self, max_cache_size: int = 1000) -> None:
        ...
        self._violation_cache: dict[str, list[PolicyViolation]] = {}  # 위반 캐시
...
class PolicyMemoryOptimizer:
    ...
    def batch_violation_operations(
        violation_groups: list[list[PolicyViolation]],
    ) -> list[PolicyViolation]:
        """위반 사항 연산들을 배치로 처리
