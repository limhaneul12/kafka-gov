...
from app.shared.constants import ActivityType, AuditAction, AuditStatus, AuditTarget, MethodType
...
class TopicBulkDeleteUseCase:
    ...
    async def execute(self, topic_names: list[TopicName], actor: str) -> dict[str, Any]:
        """
        여러 토픽을 병렬로 삭제합니다.
        ...
        async def delete_single_topic(name: TopicName) -> tuple[TopicName, bool]:
            ...
            try:
                ...
                # Apply Use Case를 통해 삭제
                result = await self.apply_use_case.execute(batch, actor)
